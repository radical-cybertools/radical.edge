...
- **Tunnel implementation** (compute→login direction): `submit_tunneled` with `tunnel=True` auto-injects `--tunnel` and `--tunnel-via <login_hostname>` into the child edge's job arguments. The child edge (started with `--tunnel`) opens the outbound SSH tunnel itself: `ssh -L 0:<bridge_host>:<bridge_port> <login_host> -N`. The OS-assigned local port is parsed from SSH's "Allocated port N" line and written to `~/.radical/edge/tunnels/<edge_name>.port` on the shared filesystem. The child then rewrites its bridge URL to `https://localhost:<port>` and connects through the tunnel. The login-side `_tunnel_watcher` no longer spawns SSH; it only monitors the batch job's state and the rendezvous file so `tunnel_status(edge_name)` can report `pending` / `active` / `failed` / `no_tunnel` (there is no longer a distinct `done`). Spawn + port parsing logic lives in `src/radical/edge/tunnel.py` and is test-covered. Login host resolution order: `--tunnel-via` CLI arg → `$PBS_O_HOST` → `$SLURM_SUBMIT_HOST`. **Why compute-initiated**: Aurora (and many PBSPro sites) blocks direct SSH from login to compute nodes; outbound compute→login SSH is universally permitted. This also removes the `pam_slurm_adopt` dependency on SLURM clusters.
...
- **iri** (`plugin_iri.py`) – IRI remote job submission (bridge-only). Submits jobs to HPC resources via the IRI REST API (no local scheduler needed). Token is sent by the client at `register_session` and held in bridge session memory only — never stored on bridge disk. Hardcoded endpoints: NERSC (`https://api.iri.nersc.gov`, Globus auth), OLCF (`https://amsc-open.s3m.olcf.ornl.gov`, S3M auth). Endpoint constants in `iri_endpoints.py`. Client API: `list_endpoints()` (session-less), `register_session(endpoint, token)`, `submit_job(resource_id, job_spec)`, `get_job_status(resource_id, job_id)`, `list_jobs(resource_id)`, `cancel_job(resource_id, job_id)`. Notification topic: `job_status` → `{job_id, state, resource_id, name, details}`. Background job poller every 10 s.
- **iri_info** (`plugin_iri_info.py`) – IRI resource information (bridge-only). Queries HPC resource status, incidents, projects, and allocations via the IRI REST API. Same hardcoded endpoints as `iri`; shares `iri_endpoints.py` constants and the `iri_tokens` localStorage key with the Explorer UI. Client API: `list_endpoints()` (session-less), `register_session(endpoint, token)`, `list_resources()`, `get_resource(resource_id)`, `list_incidents()`, `list_projects()`, `list_allocations(project_id)`.
